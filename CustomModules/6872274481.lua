...
local tweefunctionnService = game:GetService("TweenService")
...
local function runGalaxy(func) func() end
			...
			if calling then 
				table.insert(AutoToxic.Connections, vapeEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
					if AutoToxicBedDestroyed.Enabled and bedTable.brokenBedTeam.id == lplr:GetAttribute('Team') then
						local custommsg = #AutoToxicPhrases6.ObjectList > 0 and AutoToxicPhrases6.ObjectList[math.random(1, #AutoToxicPhrases6.ObjectList)] or 'Who needs a bed when you got Galaxyguard <name>?'
						if custommsg then
					...
					if deathTable.finalKill then
						...
						if killed == lplr then 
							...
							if killer == lplr and AutoToxicFinalKill.Enabled then 
								local custommsg = #AutoToxicPhrases2.ObjectList > 0 and AutoToxicPhrases2.ObjectList[math.random(1, #AutoToxicPhrases2.ObjectList)] or '<name> things could have ended for you so differently, if you\'ve used Galaxyguard. '
								if custommsg == lastsaid then
									custommsg = #AutoToxicPhrases2.ObjectList > 0 and AutoToxicPhrases2.ObjectList[math.random(1, #AutoToxicPhrases2.ObjectList)] or '<name> things could have ended for you so differently, if you\'ve used Galaxyguard.'
								else
									lastsaid = custommsg
								end
								if custommsg then
				...
				table.insert(AutoToxic.Connections, vapeEvents.MatchEndEvent.Event:Connect(function(winstuff)
					...
					if myTeam and myTeam.id == winstuff.winningTeamId or lplr.Neutral then
						...
						if AutoToxicWin.Enabled then
							sendmessage(#AutoToxicPhrases.ObjectList > 0 and AutoToxicPhrases.ObjectList[math.random(1, #AutoToxicPhrases.ObjectList)] or 'Galaxyguard is simply better everyone.')
						end
					end
				end))
...
run(function()
    ...
    function findNearestBed()
        ...
        for _, v in pairs(game.Workspace:GetDescendants()) do
            if v:IsA("BasePart") and (v.Name == "Blanket" or v.Name == "Bed") and v.BrickColor ~= lplr.Team.TeamColor then
    ...
    BedTp = exploit.Api.CreateOptionsButton({
        ...
        ["HoverText"] = "TP To Closest Bed | Works in 30v30 😱 | credits: snoopy. i fixed it"
    })
end)
...
run(function()
    ...
    local ScytheLongJump = exploit.Api.CreateOptionsButton({
        ["Name"] = "ScytheFastJump",
        ["Function"] = function(callback)
            if callback then
                currentTween = game:GetService("TweenService"):Create(
                    ...
                    TweenInfo.new(1), 
                    {Position = game.Players.LocalPlayer.Character.HumanoidRootPart.Position + game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame.lookVector * 100}
                )
                currentTween:Play()
...
AntiCheatFucker = exploit.Api.CreateOptionsButton({
	Name = "AntiCheat Disabler",
	HoverText = "Disables AntiCheat Speed/Float checks\nScythe is required",
	Function = initiateAntiCheatBypass
})
end)
